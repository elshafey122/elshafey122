# Hi, I'm Ahmed 👋
# Software engineer

## About Me
* Backend Developer specializing in .NET
* Experienced with C#, ASP.NET Core, SQL Server, Entity Framework Core
* Interested in Backend Development, System Design, Databases, and Cloud Technologies
* Frontend web development (JavaScript, TypeScript, Angular)
* Solid Principles & Design patterns
* Onion Architecture - Clean Architecture - Microservices - Monolitic - MVC 

## Tech Stack

### Backend                                                                        
C# - ASP.NET Core - Entity Framework Core - Linq - REST APIs - gRPC - SignalR - Micorservices - GraphQl - RabbitMq   
                                                                
### Frontend
Angular - JavaScript - HTML - CSS
 
### Database 
Sqlserver - mongoDb

## Tools 
Git - Github - Docker - Kubernates - Azure devops - Azure - CI/CD - Linx - Postman

## Currently Learning
SQL Server DBA Concepts -  System Design - Devops & CLoud - Azure 

## Contact Me

* LinkedIn: (https://www.linkedin.com/in/ahmed-mohamed-127391219/)
* Email:    (aelshafey135@gmail.com)

